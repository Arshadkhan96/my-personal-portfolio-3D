...
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    // Initial position (center correctly)
    gsap.set([cursor, cursorBorder], {
      xPercent: -50,
      yPercent: -50,
    });

    // Cursor movement animations (different speeds)
    const xTo = gsap.quickTo(cursor, "x", { duration: 0.2, ease: "power3.out" });
    const yTo = gsap.quickTo(cursor, "y", { duration: 0.2, ease: "power3.out" });

    const xBorderTo = gsap.quickTo(cursorBorder, "x", { duration: 0.5, ease: "power3.out" });
    const yBorderTo = gsap.quickTo(cursorBorder, "y", { duration: 0.5, ease: "power3.out" });
